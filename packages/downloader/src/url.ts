...
import { friendlyServiceName } from "./service-alias";
  ...
  switch (host.sld) {
    ...
    case "youtu":
      if (url.hostname === 'youtu.be' && parts.length >= 2) {
        ...
        ** but we only care about the 1st segment of the path */
        url = new URL(`https://youtube.com/watch?v=${encodeURIComponent(parts[1])
          }`)
      }
      break;
    ...
    case "pin":
      if (url.hostname === 'pin.it' && parts.length === 2) {
        url = new URL(`https://pinterest.com/url_shortener/${encodeURIComponent(parts[1])
          }`)
      }
      break;
