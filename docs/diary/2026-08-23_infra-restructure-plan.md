...
- [ ] **Ручной прогон в боте** — юзер проводит сам прямо сейчас/в ближайшее время:
      `/start`, `/status`, одиночный трек, повторная отправка того же URL (дедуп →
      cache hit через telegram-стор), плейлист, отдельно `SAVE_TO_CONTENT_DIR`-путь
      (имя файла `{sanitizeTitle(title)}_{trackId}.ext` в `content/mp3|mp4/`,
      fs-cache-adapter.ts ни разу не запускался)
- [ ] Смёрджить `worktree-agent-aab999315a92a7e92` в `refactor/architecture` — после
      успешного ручного прогона, решает юзер
- [ ] После мёржа — `docs/agents/planning.md`/`CLAUDE.md` всё ещё описывают дорефакторную
      структуру (`src/bot/`, `src/db/`, `src/worker/`) — обновить
- [ ] Пункты из `docs/backlog.md`, зафиксированные в этой сессии (см. выше) — не блокируют
      мёрдж, брать по одному отдельными заходами после
- [ ] Не поднимать тестовый бот-процесс с боевым `BOT_TOKEN` параллельно с Pi-инстансом —
      конфликт long-polling
