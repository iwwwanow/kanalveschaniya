...
## Разбор review-комментариев юзера — сессия 2026-08-24, продолжение

Юзер оставил 9 инлайн-заметок прямо в коде отдельным коммитом (`899b06b`,
`chore(common): review comments`, ворктри `agent-aab999315a92a7e92`) и попросил пройтись
по каждой. Заодно спросил: применяются ли миграции автоматически на старте (да —
`runAppMigrations`/`runTelegramMigrations` вызываются внутри `openAppDb()`/`openTelegramDb()`
в `main.ts`, до `bot.launch()`; `migrateLegacyDb()` там же, идемпотентно на каждый старт),
и попросил общую оценку готовности архитектуры.

**Разобрано и зафиксировано в `docs/backlog.md`** (5 новых пунктов + уточнение к
существующему "Оформление бота"):

- `blockReason` (`DownloadResult`) — три места (`yt-dlp.ts`, `process-download-job.ts`,
  `telegram-notifier.ts`) синхронизированы вручную через голый `string`, без общего типа —
  риск молчаливого рассинхрона при опечатке. Предложен `type BlockReason` рядом с инфрой
  (не в `domain/`, чтобы не тащить инфра-семантику в domain)
- `error_log`-запись в `process-download-job.ts` — единственное место в `application/`,
  где код бьёт по `Database` напрямую, в обход репозитория (решение осознанное с прошлой
  сессии, но зафиксировано как риск-прецедент для следующих похожих случаев)
- `extractUrl()` матчит, только если **всё** сообщение целиком — валидный URL; ссылка
  внутри текста ("вот трек https://...") или несколько ссылок в одном сообщении не
  находятся — реальный UX-пробел, не архитектурный
- `NotifierPort.notify()` / `TrackCachePort.deliver()` — пересечение ответственности
  (оба резолвят `jobId→chatId`, оба шлют в Telegram), было решено не трогать ещё на
  ревизии днём — здесь перенесено из дневника в бэклог как формальный открытый вопрос
- Drizzle вместо ручного `bun:sqlite` — не сейчас (не чинит текущих багов, отдельная ось
  от рефакторинга слоёв), но путь назад открыт: `drizzle-kit introspect` на существующих
  `app.db`/`telegram.db` (в т.ч. на боевых данных с Pi) даёт нулевой снапшот схемы, замена
  происходит только внутри `infra/repository/*.ts`, контракты портов в `domain/` не меняются
- Централизация UX-строк — юзер уточнил: это не про i18n, а про единую точку контроля
  ("чтобы видеть одним взглядом, что бот говорит юзеру"). Предложен `messages.ts` в
  `infra/presentation/`, объединяющий строки из `telegram-handlers.ts` и
  `telegram-notifier.ts`. Дописано в существующий пункт "Оформление бота"

**`telegram-send-media.ts`/`extract-url.ts` "is it util?"** — ответ: нет, оба
infra/presentation-специфичны (Telegram Bot API квирки конкретно, текст конкретно
телеграм-сообщения), уже на своём месте, отдельная `utils/`-папка была бы группировкой
"непохоже на остальное", а не по смыслу — решили не создавать.

**Общая оценка (по запросу юзера)**: согласился с его ощущением "80% готово". Слоистость
сделана добросовестно, `TrackStorePort`/`TrackCachePort`-ревизия и review-комментарии
показывают реальную инженерную рефлексию, а не косметику. Оставшиеся 20% — не архитектурные
дыры, а (а) перечисленные выше мелкие швы в бэклоге и (б) то, что **ни разу не было
реального прогона бота** — `fs-cache-adapter.ts` не запускался вообще, `migrate-legacy.ts`
не видел боевых данных, плейлист-fan-out не проверен на текущем коде. Typecheck чистый —
необходимое, не достаточное условие.

**Дальше юзер сказал "бота подниму сам"** — попытка поднять `bun run dev` через Claude была
прервана явно, юзер тестирует руками. `.env.local` в ворктри на момент прерывания уже
создан (тестовый бот `@iwwwanow_dev_bot`, `CHANNEL_ID=-1004480832310`) — до этого его не
было (только `.env.example`), несмотря на то что прошлая запись дневника утверждала
обратное; `data/{app,telegram}.db` в ворктри существуют, но пустые (0 строк во всех
таблицах) — прогона ещё не было.

## Ручной прогон, мёрдж и релиз 2.0.0 — сессия 2026-08-24, ночь

Юзер прогнал бота вручную (`@iwwwanow_dev_bot`, тестовый канал). Первая попытка
вскрыла реальный баг: бот на момент `save()` не был добавлен в тестовый канал →
`channel.save()` упал на `sendMedia` (`Bad Request: chat not found`) **до**
`trackRefs.save()`/`resource.save()`. Цикл сторов в `runJob` (`process-download-job.ts`)
не изолирует ошибки одного стора от других — исключение прервало весь `for`, до
`fs`-стора (архив в `CONTENT_DIR`) дело не дошло, локальная копия в этот раз не
сохранилась. После того как юзер добавил бота в канал — ретрай (30s backoff) прошёл
штатно, трек доставлен, `/status` показал `done: 1`. Разобрали по БД (`app.db`/`telegram.db`)
итоговое состояние — `queue`/`resource`/`telegram_track_refs`/`telegram_reply_refs` все
консистентны, кроме двух мелких находок (см. бэклог ниже).

**По ходу разбора добавлено логирование** — `store.name` (`"channel"`/`"fs"`) в
`domain/track-cache.ts` (`TrackStorePort.name`), и парные `save start`/`save done`/
`deliver start`/`deliver done` логи вокруг каждого стора в `process-download-job.ts` —
раньше была одна строка `storing (N backend(s))` на весь цикл, невозможно было понять
без чтения кода, на каком именно сторе и шаге упало.

**В `docs/backlog.md` добавлено 2 новых пункта** (итого 6 новых за сегодня, из них
2 — по итогам живого прогона, не ревью):

1. **Изолировать сторы друг от друга** — обернуть `save()`/`deliver()` каждого стора
   в try/catch внутри цикла, копить ошибки, перебросить агрегированно после цикла
   (чтобы retry-логика сработала как прежде), `deliver()` звать только если `save()`
   того же стора прошёл, `delivered` — по факту успеха, не по факту вызова. Юзер
   оценил как некрупную правку (не нужны отдельные очереди на стор, `fs.save()`
   идемпотентен) — сделать отдельным заходом.
2. **Мусор в `queue`** — `updateStatus(id, "done")` не чистит `error`/`block_reason`
   от предыдущей упавшей попытки (не user-facing, `/status` их не читает, но вводит в
   заблуждение при чтении БД напрямую); `queue.track_id` никогда не пишется обратно
   (`updateStatus` не принимает `trackId` в патче) — не причина повторного скачивания
   (`findDeliverable` ищет по `resource`/`telegram_track_refs`, не по `queue.track_id`),
   но лишний `getInfo()`-вызов на каждом ретрае того же job'а.

**Важное разъяснение юзеру, отдельно зафиксировать**: повторное скачивание трека при
ретрае — НЕ следствие отсутствия `queue.track_id`/`filePath` в БД. Причина — при первом
падении ни один стор не успел сохранить трек целиком, `findDeliverable()` в принципе
ищет только по `resource`/`telegram_track_refs`, независимо от состояния строки `queue`.
Это та же проблема, что уже зафиксирована в бэклоге ("Разделить скачивание и отправку")
— нет персистентного состояния "скачано, но не сохранено" между попытками одного job'а.

**Мёрдж выполнен юзером**: `worktree-agent-aab999315a92a7e92` → `refactor/architecture`
(конфликт в `docs/backlog.md` — не смысловой, оба набора пунктов независимы, склеены
без потерь) → `master`. Merge-коммит `6af331a`, `feat!: ... \n\n BREAKING CHANGE: change
architecture and schema;` — сформатирован по angular-пресету (`.releaserc.json` без
кастомного `preset`, `!`-шорткат в заголовке НЕ гарантированно триггерит major на этом
пресете, футер `BREAKING CHANGE:` — гарантированно). Перед мёржем проверено: `bun run
typecheck` чист, конфликт-маркеров не осталось нигде, секретов/`.env` в диффе нет,
`console.log`/`TODO`/`debugger` не оставлено, `package.json`/lockfile не тронуты.

**Релиз состоялся**: semantic-release поднял **major 2.0.0** (сработало на футере), образ
`ghcr.io/iwwwanow/kanalveschaniya:2.0.0` опубликован на GHCR (подтверждено скриншотом —
"Published less than a minute ago"). Единственный новый env var — `DATA_DIR` (опциональный,
не секрет, дефолт `./data`) — контролирует, где лежат `app.db`+`telegram.db` (раньше был
один файл `bot.db`, авто-миграция на первом старте не тронута этой сессией). Секретов
(`BOT_TOKEN`/`CHANNEL_ID`/`PROXY`) не прибавилось.

Сформулирован и отдан юзеру промпт для infra-repo-сессии — поднять тег образа до 2.0.0,
проверить, что PV/hostPath под данные не переехал (для авто-миграции `bot.db`), после
деплоя искать в логах `legacy migration complete: ...` и прогнать сквозной тест на
боевом боте/канале.

## Остаток работы (актуально на конец сессии 2026-08-24)

- [ ] **Инфра-деплой в прод** — юзер переходит в infra-repo проверять/поднимать тег
      образа до `2.0.0` и катить в k3s. Не проверено этой сессией (нет доступа к
      infra-репо отсюда) — см. промпт выше, который был туда передан.
