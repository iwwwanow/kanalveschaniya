...
В ветке `refactor/layers` сделано **всё, что можно сделать в этом репозитории**: рефакторинг слоёв (порты, `Resource`, enum'ы,
use-case'ы), несколько ссылок в сообщении, тексты бота в JSON, фильтр по длительности (`too_long`), fs-only cache-hit, форвард
исходного сообщения в канал, изоляция сторов, Drizzle, два этапа задачи (скачивание и доставка со своими ретраями), починка
`retryable`, тесты (`bun test`, 63 теста) и две диаграммы вместо одной большой. Релиз 3.0.0 отмечен `BREAKING CHANGE` в коммите
Drizzle (`feat(db)!`): схема БД несовместима с 2.x — **перед деплоем сделать копию `DATA_DIR`**, откат образа невозможен.

Что осталось и почему не здесь:
- **Проверить аплоад на Pi** (стрим с диска через CONNECT-прокси) — только на проде, решает пользователь.
- **Контейнеризовать telegram-прокси** и **нехватка ресурсов Pi** — репозиторий `infrastructure`.
- **Cookies для yt-dlp** — нерешён способ экспорта cookies.
- **`NotifierPort`/`ResourceCachePort` пересекаются** — осознанно отложено до третьего похожего случая; fs-only cache-hit им не стал.
- Не проверено вживую: настоящие Telegram и yt-dlp, `docker build` образа (проверены production-установка по `bun.lock` и запуск миграций
  в раскладке образа), а также поведение на Pi под нагрузкой.
...
- [x] **11. Диаграммы.** Обновить `core.d2` (`vars.d2-config.layout-engine`, `direction`, убрать/пометить
      ...
      `db`, `main` как composition root) и написать детализацию infrastructure — итог: две небольшие диаграммы `core.infrastructure.ports.d2` и `core.infrastructure.telegram.d2` вместо одной большой (три шва Telegram:
      вход, выход, хранилище/доставка). Делать после шагов 1–8 — чтобы не рисовать протечки, которые убираем.
- [x] **12. Drizzle — сделано 2026-09-20** — после шагов 2–4, когда весь SQL окажется внутри репозиториев (см. P2).
...
- [x] **Ошибки скачивания yt-dlp не ретраятся — `retryable` нигде не читается (регресс 2.0.0). Исправлено 2026-09-20:** при `retryable: true` результат уходит в ту же retry/backoff-ветку, что и исключение (`process-download-job.ts`); `retryable: false` — по-прежнему сразу `failed`. Проверено сценариями на реальной SQLite (backoff, исчерпание попыток, 404).
...
- [x] **Превентивный фильтр по длительности — сделано 2026-09-20.** Аудио, чья длительность заведомо не влезет в 50 МБ
      (`≈245 кбит/с` при `--audio-quality 0` → около 28 мин), отклоняется в `yt-dlp.ts` **до** скачивания:
      `failed` + `BlockReason.TooLong` + уведомление (`failure.too_long`). Порог выводится из лимита и переопределяется
      `MAX_TRACK_DURATION_SECONDS`; видео не фильтруется (по длительности размер не оценить); пост-проверка `too_large`
      осталась страховкой. Часовые миксы при этом качестве не проходят — адаптивный битрейт сознательно не делаем.
...
- [x] **Изолировать сторы друг от друга в `process-download-job`. Сделано 2026-09-20.** Если падает канал (например, бот не
...
- [x] (**сделано 2026-09-20**, `extractUrls`, до 10 ссылок на сообщение) `extractUrl` не находит ссылку внутри произвольного текста сообщения (только если всё сообщение —
...
- [x] Drizzle вместо ручного `bun:sqlite` (**сделано 2026-09-20**) — сознательно отложено, путь миграции описан и остаётся открытым.
...
- [x] **Форвардить в канал исходное сообщение вместе с треком — сделано 2026-09-20** (`telegram-channel-cache.save` после аплоада; best-effort; пропуск для элементов плейлиста и постов из самого канала; форварды в канале слушатель `channel_post` игнорирует). Сейчас в канал уходит только сам файл —
